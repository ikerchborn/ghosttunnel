...
Launch the control panel visually from your desktop applications menu (search for "GhostTunnel" in your launcher, just like you would search for Burp Suite, Wireshark, or other graphical security tools) or start it via the terminal:
```bash
ghostgui
```
The PyQt6 GUI connects directly to the daemon's event socket, providing real-time telemetry, a live activity log, an interactive Network Map, and one-click controls for **PANIC**, **Disable Panic**, and **Emergency Unlock**.
